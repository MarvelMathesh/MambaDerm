...
class TabularTokenEncoder(nn.Module):
    """
    Encodes tabular features as a sequence of tokens.
    
    Alternative approach where each feature becomes a token,
    allowing cross-attention with image patches.
    """
    ...
    def __init__(
        ...
        # Each numerical feature gets its own projection
        self.num_projections = nn.ModuleList([
            nn.Linear(1, token_dim) for _ in range(num_numerical)
        ])
        
        # Categorical embeddings (directly to token_dim)
        self.cat_embeddings = nn.ModuleList([
            nn.Embedding(card + 1, token_dim, padding_idx=0)
            for card in categorical_cardinalities
        ])
        
        # Learnable type embeddings to distinguish numerical vs categorical
        self.type_embed = nn.Parameter(torch.randn(2, token_dim) * 0.02)
        
        # Position embeddings for the tabular tokens
        total_tokens = num_numerical + num_categorical
        self.pos_embed = nn.Parameter(torch.randn(total_tokens, token_dim) * 0.02)
    ...
    def forward(
        ...
        Returns:
            Token sequence of shape (B, num_tokens, token_dim)
        """
        batch_size = numerical.shape[0]
        tokens = []
        
        # Numerical features as tokens
        numerical = torch.nan_to_num(numerical, nan=0.0)
        for i, proj in enumerate(self.num_projections):
            token = proj(numerical[:, i:i+1])  # (B, token_dim)
            token = token + self.type_embed[0]  # Add numerical type embedding
            tokens.append(token)
        
        # Categorical features as tokens
        if categorical is not None:
            for i, embed_layer in enumerate(self.cat_embeddings):
                cat_idx = categorical[:, i].long().clamp(0, embed_layer.num_embeddings - 1)
                token = embed_layer(cat_idx)  # (B, token_dim)
                token = token + self.type_embed[1]  # Add categorical type embedding
                tokens.append(token)
        
        # Stack tokens
        tokens = torch.stack(tokens, dim=1)  # (B, num_tokens, token_dim)
